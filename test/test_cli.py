...
@pytest.mark.parametrize(
    "args, output_expected, clipboard_expected",
    [
        (
            ["laptop", "bob"],
        ...
        (["--echo", "laptop", "bob"], "laptop: bob | b0b", CLIPBOARD_NOT_TOUCHED),
        (["--echo", "--user", "laptop", "bob"], "laptop: bob", CLIPBOARD_NOT_TOUCHED),
        (["--raw", "laptop", "bob"], "b0b", CLIPBOARD_NOT_TOUCHED),
        (["--raw", "--user", "laptop", "bob"], "bob", CLIPBOARD_NOT_TOUCHED),
    ],
)
...
@pytest.mark.parametrize(
    "args, use_clipboard, length_expected",
    [
        (["--gen"], True, pw.__main__.RANDOM_PASSWORD_DEFAULT_LENGTH),
        (["--gen", "--copy"], True, pw.__main__.RANDOM_PASSWORD_DEFAULT_LENGTH),
        (["--gen", "--echo"], False, pw.__main__.RANDOM_PASSWORD_DEFAULT_LENGTH),
        (
            ["--gen", "--echo", "--raw"],
            False,
...
def test_gen(runner, args, use_clipboard, length_expected):
    ...
    if use_clipboard:
        ...
        assert all(c in pw.__main__.RANDOM_PASSWORD_ALPHABET for c in clipboard)
    else:
...
def test_edit_with_changes(dirname, filename, addendum):
    ...
    try:
        ...
        os.environ["PW_EDITOR"] = '%s "%s" "%s"' % (sys.executable, editor, addendum)
        os.environ["PW_GPG_RECIPIENT"] = "test.user@localhost"
        runner = CliRunner()
        ...
        assert len(result) == 1
        assert result[0] == pw.Entry(
            "fancy_new_entry", "user", "pass", "interesting notes"
        )
    finally:
        os.unlink(fp.name)
